...
class kWTA_Spike(nn.Module):
    def __init__(
        ...
        k_min=10, soft_reset=True, detach_nonspike=False, k_param=50, k_approach="LINEAR_DECAY"
    ):
...
class kWTA_Spk_tr(neuron.BaseNode):
    def __init__(
        self, surr_func=surrogate.Sigmoid(), neu_size=None, neu_type='if', device=None,
        # -> parameter for k-WTA
        soft_reset=True, k_min=10, inhibit_intense=1.0, use_inhibit=True, inhibit_self=False, hard_mask=True,
        # -> parameter for specific excite and inhibit
        last_inhibit=False, curr_excite=False, tune_approach="sigmoid", inhibit_p=1e-4, excite_p=1e-4,
        tune_param=1., adapt_tune=True,
        # -> parameter for trace
        tr_refresh=False, tr_decay=10., one_size_tr=False,
        ...
        self.neu_tr = None
        self.tr_refresh = tr_refresh  # -> whether the trace need to refresh every batch
        self.one_size_tr = one_size_tr # -> whether to keep only one trace
        self.tr_decay = tr_decay

        # ->parameter about task-specific
        ...
        self.adapt_tune = adapt_tune  # -> whether to tune the k
        if tune_approach == "sigmoid":
            self.tr_tune = self.tr_sigmoid
        elif tune_approach == "tanh":
            self.tr_tune = self.tr_tanh
        elif tune_approach == "linear":
            self.tr_tune = self.tr_linear
        else:
            raise NotImplementedError(f"don't have the activation tunning function{tune_approach}")
        ...
        tr_label = "-{tr_size}_{tr_refresh}_{tr_decay}{inh}".format(
            tr_size="one" if self.one_size_tr else "ba",
            tr_refresh="b-ref" if self.tr_refresh else "t-ref",
        ...
        k_label = "-{approach}{parameters}".format(
            approach="hard_" if self.hard_mask else "soft_", parameters=f"k{self.k_min}",
        )
        self.label = f"-kspktr{neu_label}{k_label}{tr_label}{specific_label}"
    ...
    def top_k_fire(self, curr_ep=None, training=True, final_step=False):
        ...
        if self.neu_tr is None:
            if self.one_size_tr and training:
                self.neu_tr = torch.zeros(*self.v.shape[1:]).to(self.v.data.device)
            else:
                self.neu_tr = torch.zeros_like(self.v).to(self.v.data.device)

        spike = self.surrogate_function(self.v - self.v_threshold)
        append_tr = torch.sum(spike.detach(), dim=0) if self.one_size_tr and training else spike.detach()
        self.neu_tr = self.neu_tr - self.neu_tr / self.tr_decay + append_tr

        tuned_tr = self.tr_tune(self.neu_tr) if training else self.neu_tr
        ...
        if self.hard_mask or not final_step:
            ...
            vals, inds = torch.topk(tuned_tr, curr_k + 1, dim=-1, sorted=False)
            ...
            vals, inds = torch.topk(tuned_tr[..., rand_ind], curr_k, dim=-1, sorted=False)
        ...
        if self.one_size_tr and training:
            top_k_mask = top_k_mask.repeat(self.v.shape[0], 1)
    ...
    def multi_step_forward(self, x_seq: torch.Tensor, curr_ep=None, training=True, **kwargs):
        ...
        if self.tr_refresh or not training:
            self.neu_tr = None
    ...
    def end_one_task(self):
        self.last_act_cnts += self.neu_act_cnts
        self.neu_act_cnts = torch.zeros_like(self.neu_act_cnts)
        if not self.tr_refresh:
            self.neu_tr = None
        return None

    def tr_sigmoid(self, trace):
        summed_param = self.neu_act_cnts * self.excite_p - self.last_act_cnts * self.inhibit_p
        return trace * (torch.sigmoid(summed_param) * self.tune_param + (1. - self.tune_param/2))

    def tr_tanh(self, trace):
        summed_param = self.neu_act_cnts * self.excite_p - self.last_act_cnts * self.inhibit_p
        return trace * (1 + torch.tanh(summed_param) * self.tune_param / 2)

    def tr_linear(self, trace):
        summed_param = self.neu_act_cnts * self.excite_p - self.last_act_cnts * self.inhibit_p
        summed_param = torch.minimum(
            torch.ones_like(self.last_act_cnts) * (1. + self.tune_param/2),
            torch.maximum(
