...
def get_confusion_matrix(args, seed, save_root, type_name, recover_act=True):
    fig_name = type_name + f'_confusion_s{seed}.png'
    ...
    utils.load_checkpoint(model, args.model_dir, name=load_name, verbose=True, recover_act=recover_act)
...
def save_specific_neuron_distribute(method_dict, seed, type_name, save_root, item_name='total', ylim=None):
    ...
    fig_name = type_name + f'_regr_s{seed}.png'
    if os.path.isfile(os.path.join(save_root, fig_name)):
        print(f"{fig_name}: already exist")
        return
...
if __name__ == '__main__':
    ...
    get_confusion_matrix(args, seed=seed_list[0], save_root=args.plot_dir, type_name='snn_mask', recover_act=True)
    ...
    get_confusion_matrix(args, seed=seed_list[0], save_root=args.plot_dir, type_name='snn_step', recover_act=True)
