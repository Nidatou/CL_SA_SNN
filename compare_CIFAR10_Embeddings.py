...
if __name__ == '__main__':
    ...
    # args.thresh_min = 1.
    # WTK_M_EWC_solid_1 = {}
    # WTK_M_EWC_solid_1 = collect_all(WTK_M_EWC_solid_1, seed_list, args, name="WTK_M_EWC")
    #
    # args.use_mask = True
    # args.ewc = False
    # args.norm_const = 1.
    ...
    for seed in seed_list:
        ...
        task_acc[seed] = [
            # SDM[seed][i]['all_tasks'][task_id] for task_id in range(task_num)
            NONE[seed][i]['all_tasks'][task_id] for task_id in range(task_num)
            # WTK_S[seed][i]['all_tasks'][task_id] for task_id in range(task_num)
        ]
        i = 1
        ave_acc[seed] = [
    ...
    for seed in seed_list:
        act_k_list[seed] = [
            WTK_M[seed][0]['act_k'],
            WTK_S[seed][0]['act_k'],
            WTK_M_EWC[seed][0]['act_k'],
            WTK_S_EWC[seed][0]['act_k'],
        ]
        sparseness_list[seed] = [
            WTK_M[seed][0]['sparseness'],
            WTK_S[seed][0]['sparseness'],
            WTK_M_EWC[seed][0]['sparseness'],
            WTK_S_EWC[seed][0]['sparseness'],
        ]
    ...
    names = ["Joint", "None", "EWC", "MAS", "FlyModel", "SDM", 'WTK_M', 'WTK_S', "SDM_EWC", 'WTK_S_EWC']
    colors = ["black", "grey", "#CC79A7", "goldenrod", "#56B4E9", 'green', 'blue', 'goldenrod', 'yellowgreen', 'red']
    ...
    for task_id in range(task_num):
        ...
        p_size = len(new_tasks_acc) // task_num
        print(f"p_size is {p_size}")
        print(f"The task_{task_id}'s performance is {np.array(new_tasks_acc[(p_size - 1)::p_size])}, "
              f"and sems is +/- {np.array(new_tasks_sem[(p_size - 1)::p_size])}")
    ...
    methods_len = len(act_k_list[seed_list[0]])
    means_actk = [np.mean([act_k_list[seed][id] for seed in seed_list]) for id in range(methods_len)]
    means_sparsenees = [np.mean([sparseness_list[seed][id] for seed in seed_list]) for id in range(methods_len)]
    if args.n_seeds > 1:
        sems_actk = [np.sqrt(np.var([act_k_list[seed][id] for seed in seed_list])/(len(seed_list) - 1)) for id in range(methods_len)]
        sems_sparse = [np.sqrt(np.var([sparseness_list[seed][id] for seed in seed_list])/(len(seed_list)-1)) for id in range(methods_len)]
    act_names = ['WTK_M', 'WTK_S', "WTK_M_EWC", 'WTK_S_EWC']
    for i, name in enumerate(act_names):
        if len(seed_list) > 1:
            print("{:30s} act_num {:5.2f}  (+/- {:4.2f}), n={}".format(name, means_actk[i], sems_actk[i], len(seed_list)))
            print("{:30s} sparseness {:5.2f}  (+/- {:4.2f}), n={}".format(name, means_sparsenees[i], sems_sparse[i], len(seed_list)))
        else:
            print("{:30s} act_num {:5.2f}".format(name, means_actk[i]))
            print("{:30s} sparseness {:5.2f}".format(name, means_sparsenees[i]))
